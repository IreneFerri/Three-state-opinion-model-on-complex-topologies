...
# MAIN ##########################
###############################################################
alpha_str = '0.75'  # Step parameter (it only affects the dynamics speed)
alpha_name = '0.75'
alpha = 0.75
reps = 1000  # Number of repetitions for statistics
length = reps # Input array size
net_name = 'yotambien'
com_file = net_name+'.net'
###########################################################
#
# Orientations
##########################################################
...
N = G.number_of_nodes()
print('N = ', N)
filename = 'orientations_yotambien_N'+str(N).zfill(4)+'_alpha'+"%04.2f"%alpha+'_T0.144'+'.csv' # Input file
...
#  PLOT  HEATMAP SQUARE
###############################################################
fig_name = '../../Desktop/correlation/cij_'+net_name+'_N'+str(N)+'_a'+alpha_name+'.png'  # Output name
#
label_fontsize = 25
...
plt.text(2000, 2000, '(b)', fontsize=25)
#
# Tics -----------------------------
plt.yticks(np.arange(0, 2500, 1000), fontsize=25)
plt.xticks(np.arange(0, 2500, 1000), fontsize=25)
