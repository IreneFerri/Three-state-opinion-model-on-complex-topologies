...
!          integer :: my_index
          ...
          do i = last_1+1, last0
            ...
            enddo
          enddo
          ene = ene - (alpha*summ_bis)! -------------- contribution of 0 spins
          do i = last0+1, N
            do j = Pini(nodes_vec(i)), Pfin(nodes_vec(i))
              call findindex(N, nodes_vec, V(j), 1, N, index_neighbor)
              if (index_neighbor <= last_1) then
            ...
            enddo
          enddo 
          ene = ene - float(summ1)! -------------- contribution of +1 spins
! -----------------------------------------------------------------------------
...
!
      ...
      OPEN(UNIT=10,FILE='z1.dat')        ! Input
      READ(10,DADES)
      CLOSE(10)
!
...
! Markov chain -------------------------------------------------------------------------------------------           
          DO imc = 1,MCtot
            DO ISTEP = 1,N         ! Each MC has N flip attempts
              ...
              a = floor(N*r1279()) + 1  ! Pick a random spin
...
!     
        ...
        vare=f_energy2-(f_energy*f_energy)
        var0=f_num02-(f_num0*f_num0)
        var1=f_num12-(f_num1*f_num1)
        var_1=f_num_12-(f_num_1*f_num_1)
        varx=f_numx2-(f_numx_abs*f_numx_abs)
! -------------------------------------------------------------------------
!     WRITING
! ---------------------------------------------------------------------
